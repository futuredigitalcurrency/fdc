...
// Copyright (c) 2011-2012 futuredigitalcurrency Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("futuredigitalcurrency-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #futuredigitalcurrencyTEST0\r");
            Send(hSocket, "WHO #futuredigitalcurrencyTEST0\r");
        } else {
            // randomly join #futuredigitalcurrency00-#futuredigitalcurrency99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #futuredigitalcurrency%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #futuredigitalcurrency%02d\r", channel_number).c_str());
        }
