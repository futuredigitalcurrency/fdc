...
uint256 hashGenesisBlock("0x0525682d4f56ca2a656baced4f70e3a7ca2dfadba8c15ee0f1f053f37f8351aa");
...
const string strMessageMagic = "futuredigitalcurrency Signed Message:\n";
...
{
    // futuredigitalcurrency: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetSpacing = 3.33 * 60;
...
{
    // futuredigitalcurrency: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "fdc genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    assert(block.hashMerkleRoot == uint256("0x9bdd132915324e5f63664306344680f21b3f3f39b193cc50df04cb71d2f5623e"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    //// debug print
    printf("futuredigitalcurrencyMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("futuredigitalcurrencyMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("futuredigitalcurrencyMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static futuredigitalcurrencyMiner(CWallet *pwallet)
{
    printf("futuredigitalcurrencyMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("futuredigitalcurrency-miner");
    ...
    try { loop {
        ...
        printf("Running futuredigitalcurrencyMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("futuredigitalcurrencyMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&futuredigitalcurrencyMiner, pwallet));
}
